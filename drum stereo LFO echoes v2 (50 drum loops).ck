...
[.5,.5,.25,.25,.125,.125,.25,.5] @=> float beatChop[];
...
spork~kickPlay(1);//kick per beat
...
while (now < future) {
    ...
    if (random == 1) {
        ...
        beatChop[t % beatChop.cap()]*beat => now;
        } 
    t++;
}
...
fun void kickPlay(int kickPerBeat) {
    while (true) {
        0 => kick.pos;
        beat/kickPerBeat => now;
    }   
