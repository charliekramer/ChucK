60./120.=> float beatsec; //just to synchbeat - (now % beat) => now;
...
Std.mtof(36) => wurley.freq => float baseFreq; // base frequency for the ascending/descending notes
0.2 => wurley.gain; //wurley parameters
.1 => wurley.lfoSpeed;
0.9 => wurley.lfoDepth;
...
1.4 => float freqMult; //also try 1.4, 1.5,1.8; .9, .99,.5 for weird results
5 => int iMax;
5 => int jMax;
